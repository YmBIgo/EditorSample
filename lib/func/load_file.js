// 
// ToDo List
// 	 1 : check path when save index.
// 

var fs 		= require("fs");
// var node_parser = require("node-html-parser");
...
/*
path / index / depth / file|folder / current_id / parent_id /
*/
...
function readFiles(file_path, file_index, file_depth, file_result, is_directory, parent_file_index=1, is_root_file=0){
	...
	fs.readdirSync(file_path).forEach( function(file) {
		...
		if ( file_depth >= 100 ) {
			...
			var each_file_stat; var each_file_path_fixed = each_file_path;
			try {
				// [ Amazon Web Service ]
				// 
				// if ( each_file_path.slice(-1) == "/" ) {
				// 	 each_file_path_fixed = each_file_path.slice(0, -1);
				// }
				// console.log("loading file : ", each_file_path_fixed);
				// 
				each_file_stat = fs.lstatSync(each_file_path_fixed);
				if (each_file_stat.isFile()) {
					file_result.push([file, file_index, file_depth, 1, index_number, parent_file_index]);
				} else if (each_file_stat.isDirectory()) {
...
// Read Modal Data
function readModal(file_path){
	var modal_file_path = file_path;
	var modal_content = fs.readFileSync(modal_file_path, 'utf-8');
	return modal_content
}
// Read Search Data
function getCodeFromPage(page_content){
	// 
	// [ ClassName ]
	// 	 * SyntaxHighlighter => code | language | 
	// 	 * teratail			 => <code> language-[JavaScript]
	var class_regexp1 	= /class\=\"[\s\w]\"/
	var class_regexp2	= /class\=\'[\s\w]\'/
	var id_regexp1		= /id\=\"[\s\w]\"/
	var id_regexp2		= /id\=\'[\s\w]\'/
	// 
}
...
function ReadFullFile(file_path, file_array, file_limit=100, file_depth=0){
	// 
	if ( file_limit == 0 ) { return file_array }
	if ( file_depth == 1 ) {
		// file path check
		console.log("Read Full File ", file_path);
		var file_path_start   = file_path.replace("./", "");
		if ( file_path_start.slice(0, 10) == "temp_files" ) { return file_array }
		if ( file_path_start.slice(0, 5)  == "files" ) { return file_array }
	}
	var all_contents  = fs.readdirSync(file_path);
	// if ( all_contents.length > 20 ) { file_limit = 2 } // 全部読み込むようにする
	...
	all_contents.forEach(function(content){
		...
		if ( is_file_dir ) {
			...
			var next_file_depth = file_depth + 1;
			file_array = ReadFullFile(gen_file_path, file_array, next_file_limit, next_file_depth);
		} else {
			// 
...
function saveFileIndexJson(file_path){
	// should check file_path in order not for user
	// to invoke suspicious path or code
	// 
	...
	result_file_arrays = ReadFullFile(file_path, file_arrays, 30, 0);
	...
	result_file_arrays.forEach(function(file_path_ele, index){
		...
		if ( index % result_file_arrays_len_percent == 0 ) {
			...
			console.log("File indexed ", result_percent, "% ...");
		}
...
// Search Code
module.exports.get_code_from_page	= getCodeFromPage;
// Generate file
