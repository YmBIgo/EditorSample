...
function readFile(file_path, file_number, file_array, is_gen_file=false){
	...
	f_file_path = replaceAll(f_file_path, "..", "");
...
function readTempfile(file_path){
	...
	var temp_file_path 		 = replaceAll(temp_file_path, "..", "");
	...
	temp_file_path = replaceAll(temp_file_path, "//", "/");
...
function generateFile(file_path, file_content){
	...
	// .. // の対策
	var fixed_file_path = replaceAll(file_path, "..", ".");
	fixed_file_path = replaceAll(fixed_file_path, "//", "/");
...
function generate_file_path(file_path, is_temp=0){
	...
	var result_file_path  = is_file_generated ? file_path : replaceAll((file_path_format + file_path), "//", "/");
...
function ReadFullFile(file_path, file_array){
	// 
	...
	all_contents.forEach(function(content){
		...
		if ( is_file_dir ) {
			file_array = ReadFullFile(gen_file_path, file_array);
		}
...
function saveFileIndexJson(file_path){
	...
	result_file_arrays.forEach(function(file_path_ele){
		// 
		var extension_name = file_path_ele.split(".").slice(-1)[0];
		extension_name = (extension_name != undefined) ? replaceAll(extension_name, "/", "") : "";
		if ( file_extension_array.includes(extension_name) ) {
			// eval たぶん 危険じゃない ... ?
			var file_content = fs.readFileSync(file_path_ele, "utf-8");
			var file_analyze_code 	= "simple_indexer.analyze_" + extension_name + "_file(file_content, file_path_ele);";
