// 
// SQL injection 対策に SET には ? とか使うべきか
// 
'use strict';

const sqlite3 = require("sqlite3").verbose();

module.exports = class SqliteConnector {
	constrctor() {
		this.sqlite3_db;
	...
	// [ Select Data ]
	select_db_data(select_cols, condition_array, db_table_name="users", default_cols, return_sqlite_connection){
		// select data > array
		if ( Array.isArray(default_cols) != true ) { default_cols = this.default_user_cols() }
		const enable_default_array = default_cols;
		...
		return new Promise (function (resolve, reject){
			...
			return_sqlite_connection.sqlite3_db.all(select_table_query, function(err, rows){
				if (err) {
					throw reject(err);
				} else {
					resolve({rows: rows});
...
// var user_default_array 	  = sqlite_connection.default_user_cols();
// var select_user_data = sqlite_connection.select_db_data(select_user_cols, select_user_condition, "users", user_default_array, sqlite_connection);
...
// var search_history_default_array	= sqlite_connection.default_search_history_cols();
// var select_search_history_data 		= sqlite_connection.select_db_data(select_search_history_cols, select_search_history_condition, "search_histories", search_history_default_array, sqlite_connection);
...
// var select_project_data 	 = sqlite_connection.select_db_data(select_project_cols, select_project_condition, "projects", project_default_array, sqlite_connection);
